...
SCENARIO("25"){
  std::cerr << "25 - prepareModels - clearCommand - empty model" << std::endl;
...
SCENARIO("26"){
  std::cerr << "26 - prepareModels - drawCommand - inserting roots" << std::endl;
  ...
  auto check = checkTranslation(model,Diff::NOF_VERTICES);
  ...
  Chyba je tato:)." << std::endl;
  std::cerr << translatedModelErrors(2,check) << std::endl;
  REQUIRE(false);
}

SCENARIO("27"){
  std::cerr << "27 - prepareModel - drawCommand - mesh attributes & indexing" << std::endl;
...
SCENARIO("28"){
  std::cerr << "28 - prepareModel - traverse - check roots" << std::endl;
...
SCENARIO("29"){
  std::cerr << "29 - prepareModel - traverse - node in node" << std::endl;
...
SCENARIO("30"){
  std::cerr << "30 - prepareModel - traverse - binary tree" << std::endl;
...
SCENARIO("31"){
  std::cerr << "31 - prepareModel - traverse - tree" << std::endl;
