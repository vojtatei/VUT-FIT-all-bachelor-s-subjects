...
void Stack_Init( Stack *stack ) {
	if (stack == NULL)
	{
		Stack_Error(SERR_INIT); //Chyba: prázdný zásobník
	}
	else
	{
		stack->topIndex = -1;
		...
		if (stack->array == NULL)
		{
			Stack_Error(SERR_INIT); //Chyba: Nepodařilo se alokovat paměť pro zásobník
		}
	}
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	...
	{
		Stack_Error(SERR_TOP); // Chyba: Nelze provést Top na prázdném zásobníku
	}
	else
...
void Stack_Push( Stack *stack, char data ) {
	...
	{
		Stack_Error(SERR_PUSH); // Chyba: Zásobník je plný, nelze provést Push.
	}
	else
	{
		stack->topIndex++;
